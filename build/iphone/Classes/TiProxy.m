...
//Rare exceptions to indicate a bug in the _sirvi code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_sirvi = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_sirvi]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.sirvi.module"
