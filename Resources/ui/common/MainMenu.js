...
Titanium.Geolocation.addEventListener('location', locationCallback);
    ...
    shrinkBtn = Ti.UI.createAnimation({
        ...
        opacity:1,
        duration:500,
        center:{x:'93dp',y:Ti.Platform.displayCaps.platformHeight/2},
    });
    
    callBtn.animate(sunAnimation, function(e){
        childView.animate(planetAnimation);
        healthBtn.animate({right:'26.5dp', top:'59dp', duration:1000,curve:Ti.UI.ANIMATION_CURVE_EASE_IN_OUT}); 
        datingBtn.animate({right:'0dp', bottom:'158dp', duration:1000,curve:Ti.UI.ANIMATION_CURVE_EASE_IN_OUT});
        
        financeBtn.animate({right:'26.5dp', bottom:'59dp', duration:1100,curve:Ti.UI.ANIMATION_CURVE_EASE_IN_OUT}); 
        raffleBtn.animate({bottom:'0dp', left:'20dp', opacity:1 ,duration:1100,curve:Ti.UI.ANIMATION_CURVE_EASE_IN_OUT}); 
    });
    ...
    datingBtn.addEventListener('click',function(e){
        ...
        datingBig.center={x:dLeft,y:'50%'};
        self.add(datingBig);
        datingBtn.hide();        
        datingBig.animate(planetZoom);
        childView.animate(planetRotateOut, function(){
            Ti.fireEvent('lawview',{x:'date'});
        });
        callBtn.animate(shrinkBtn);
        }
    });
